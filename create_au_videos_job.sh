...
# Execute this with : sbatch --account none create_au_videos_job.sh

############# SLURM SETTINGS #############
#SBATCH --job-name=create_au_videos          # some descriptive job name of your choice
#SBATCH --output=%x-%j.out                   # output file name will contain job name + job ID
#SBATCH --error=%x-%j.err                    # error file name will contain job name + job ID
#SBATCH --time=04-0:00:00                     # time limit for the whole run, in the form of d-hh:mm:ss, also accepts mm, mm:ss, hh:mm:ss, d-hh, d-hh:mm
...
python -u create_au_videos.py -u pa121h
